...
export interface TaskRecord {
  ...
  tags: string[];
  // AIT42 v1.5.0: Git diff tracking for regression prevention
  git_diff?: {
    staged: string;                    // git diff --staged (changes before task)
    unstaged: string;                  // git diff (working tree changes)
    branch: string;                    // Current branch
    constraints_detected: string[];     // Auto-extracted constraints from diff
    timestamp: string;                  // When diff was captured
  };
